...
void lecture(int & n, double & tf, string & test, double & disc_x, double & disc_y, vector<double> & rho, vector<double> & u, vector<double> & v, vector<double> & p, int & choix_schema, int & choix_ordre){
    ...
    std::ifstream monFlux("initialisation.data", std::ios::in);
...
// Calcul erreur
void error(vector<vector<double>> U_rho, vector<vector<double>> U_rhou, vector<vector<double>> U_rhov, vector<vector<double>> U_E, vector<vector<double>> U_rho_exact, vector<vector<double>> U_rhou_exact, vector<vector<double>> U_rhov_exact, vector<vector<double>> U_E_exact, string result, int n, double dx, double dy, double gamma, double b, int fichier){

    string ignore;
    fichier =1;
    double delta_x(0), delta_y(0), density(0), vx(0), vy(0), pressure(0), energy(0), e_int(0), e_intx(0), e_inty(0), e_int_exact(0), e_int_exactx(0), e_int_exacty(0);
    double error_rho(0), error_u(0), error_v(0), error_p(0), error_e(0);
    int kx, ky;
    double xi, yi, x0(0), y0(0);
    vector<double> U_int(4,0), U_int_exact(4,0), U_intx(4,0), U_int_exactx(4,0), U_inty(4,0), U_int_exacty(4,0);

    for (int i = 2; i < n/fichier-3; i++)
    // for (int i = 0; i < n; i++)
    {
        kx=i*fichier;
        xi = x0 + kx*delta_x;
        for (int j = 2; j < n/fichier-3; j++)
        // for (int j = 0; j < n; j++)
        {
            ky=j*fichier;
            yi = y0 + ky*delta_y;
         ...
         // ky = n/2;
            ...
            U_int_exact[0] = U_rho_exact[kx][ky];
            U_int_exact[1] = U_rhou_exact[kx][ky];
            U_int_exact[2] = U_rhov_exact[kx][ky];
            U_int_exact[3] = U_E_exact[kx][ky];

            U_intx[0] = U_rho[kx+1][ky];
            U_intx[1] = U_rhou[kx+1][ky];
            U_intx[2] = U_rhov[kx+1][ky];
            U_intx[3] = U_E[kx+1][ky];

            U_int_exactx[0] = U_rho_exact[kx+1][ky];
            U_int_exactx[1] = U_rhou_exact[kx+1][ky];
            U_int_exactx[2] = U_rhov_exact[kx+1][ky];
            U_int_exactx[3] = U_E_exact[kx+1][ky];

            U_inty[0] = U_rho[kx][ky+1];
            U_inty[1] = U_rhou[kx][ky+1];
            U_inty[2] = U_rhov[kx][ky+1];
            U_inty[3] = U_E[kx][ky+1];

            U_int_exacty[0] = U_rho_exact[kx][ky+1];
            U_int_exacty[1] = U_rhou_exact[kx][ky+1];
            U_int_exacty[2] = U_rhov_exact[kx][ky+1];
            U_int_exacty[3] = U_E_exact[kx][ky+1];

            error_rho += (dx/2)*(pow(U_rho[kx][ky] - U_rho_exact[kx][ky], 2) + pow(U_rho[kx+1][ky] - U_rho_exact[kx+1][ky], 2)) + (dy/2)*(pow(U_rho[kx][ky] - U_rho_exact[kx][ky], 2) + pow(U_rho[kx][ky+1] - U_rho_exact[kx][ky+1], 2)) ;
            error_u += (dx/2)* (pow(U_rhou[kx][ky]/U_rho[kx][ky] - U_rhou_exact[kx][ky]/U_rho_exact[kx][ky], 2) + pow(U_rhou[kx+1][ky]/U_rho[kx+1][ky] - U_rhou_exact[kx+1][ky]/U_rho_exact[kx+1][ky], 2)) + (dy/2)* (pow(U_rhou[kx][ky]/U_rho[kx][ky] - U_rhou_exact[kx][ky]/U_rho_exact[kx][ky], 2) + pow(U_rhou[kx][ky+1]/U_rho[kx][ky+1] - U_rhou_exact[kx][ky+1]/U_rho_exact[kx][ky+1], 2));
            error_v += (dx/2)* (pow(U_rhov[kx][ky]/U_rho[kx][ky] - U_rhov_exact[kx][ky]/U_rho_exact[kx][ky], 2) + pow(U_rhov[kx+1][ky]/U_rho[kx+1][ky] - U_rhov_exact[kx+1][ky]/U_rho_exact[kx+1][ky], 2)) + (dy/2)* (pow(U_rhov[kx][ky]/U_rho[kx][ky] - U_rhov_exact[kx][ky]/U_rho_exact[kx][ky], 2) + pow(U_rhov[kx][ky+1]/U_rho[kx][ky+1] - U_rhov_exact[kx][ky+1]/U_rho_exact[kx][ky+1], 2));
            error_p += (dx/2)* (pow(pression(U_int, gamma) -pression(U_int_exact,gamma), 2) + pow(pression(U_intx, gamma) -pression(U_int_exactx,gamma), 2)) + (dy/2)* (pow(pression(U_int, gamma) -pression(U_int_exact,gamma), 2) + pow(pression(U_inty, gamma) -pression(U_int_exacty,gamma), 2));
            e_int = (pression(U_int, gamma)*(1-b*U_int[0]))/((gamma-1)*U_int[0]);
            e_intx = (pression(U_intx, gamma)*(1-b*U_intx[0]))/((gamma-1)*U_intx[0]);
            e_inty = (pression(U_inty, gamma)*(1-b*U_inty[0]))/((gamma-1)*U_inty[0]);
            e_int_exact = (pression(U_int_exact, gamma)*(1-b*U_int_exact[0]))/((gamma-1)*U_int_exact[0]);
            e_int_exactx = (pression(U_int_exactx, gamma)*(1-b*U_int_exactx[0]))/((gamma-1)*U_int_exactx[0]);
            e_int_exacty = (pression(U_int_exacty, gamma)*(1-b*U_int_exacty[0]))/((gamma-1)*U_int_exacty[0]);
            error_e += (dx/2) * (pow(e_int - e_int_exact,2) + pow(e_intx - e_int_exactx,2)) + (dy/2) * (pow(e_int - e_int_exact,2) + pow(e_inty - e_int_exacty,2));

        }
    }

    ofstream mon_flux1;
    mon_flux1.open("Resultats/erreur_" + result, ios::app);
    mon_flux1 << dx << " " << sqrt(error_rho*dx*dy/2) << " " << sqrt(error_u*dx*dy/2) << " " << sqrt(error_v*dx*dy/2) << " " << sqrt(error_p*dx*dy/2) << " " << sqrt(error_e*dx*dy/2) << endl;
    mon_flux1.close();
}
